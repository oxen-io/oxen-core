...
void match_string2(const char*& star_end_string, const char* buf_end, std::string& val) {
                    ...
                    if (buf_end - it < 4) {
                        ...
                        for (int i = 0; i < 4; ++i) {
                            ...
                            CHECK_AND_ASSERT_THROW_MES(
                                    oxenc::is_hex_digit(c), "Bad Unicode encoding: {}", c);
    ...
    ASSERT_MES_AND_THROW(
            "Failed to match string in json entry: {}", std::string_view{star_end_string, buf_end});
}
// The only conclusive thing that can be said about this function is that it does indeed
// closely match a "number 2". 💩
void match_number2(
        const char*& star_end_string,
        const char* buf_end,
        std::string_view& val,
        bool& is_float_val,
        bool& is_negative_val) {
    val = {};
    ...
    for (; it != buf_end; it++) {
        ...
        if (flags & 16) {
            ...
            if (val.size()) {
                ...
                ASSERT_MES_AND_THROW(
                        "wrong number in json entry: {}", std::string(star_end_string, buf_end));
        }
    }
...
void match_word2(const char*& star_end_string, const char* buf_end, std::string_view& val) {
    ...
    ASSERT_MES_AND_THROW(
            "failed to match word number in json entry: {}", std::string(star_end_string, buf_end));
}
