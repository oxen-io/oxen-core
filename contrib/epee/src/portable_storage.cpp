...
namespace epee { namespace serialization {
    ...
    void dump_as_json(std::ostream& strm, const storage_entry& se, size_t indent, bool pretty) {
        var::visit([&](const auto& v) { dump_as_json(strm, v, indent, pretty); }, se);
    }
    ...
    void dump_as_json(std::ostream& s, const std::string& v, size_t, bool) {
        ...
        // (we could also use \b, \f, \r, but it really isn't worth the bother.
        for (char c : v) {
            switch (c) {
                case '"':
                case '\\':
    ...
    bool portable_storage::load_from_binary(const epee::span<const uint8_t> source) {
        ...
        TRY_ENTRY();
        throwable_buffer_reader buf_reader(
                source.data() + sizeof(storage_block_header),
                source.size() - sizeof(storage_block_header));
        buf_reader.read(m_root);
        return true;  // TODO:
        CATCH_ENTRY("portable_storage::load_from_binary", false);
    }

    section* portable_storage::open_section(
            const std::string& section_name, section* parent_section, bool create_if_notexist) {
        TRY_ENTRY();
        if (!parent_section)
    ...
    bool portable_storage::get_value(
            const std::string& value_name, storage_entry& val, section* parent_section) {
        // TRY_ENTRY();
        if (!parent_section)
            parent_section = &m_root;
    ...
    storage_entry* portable_storage::find_storage_entry(
            const std::string& pentry_name, section* psection) {
        TRY_ENTRY();
    ...
    section* portable_storage::insert_new_section(
            const std::string& pentry_name, section* psection) {
        TRY_ENTRY();
