...
You should verify that this is really the checksum you get on that file you built. Loki is currently planning to switch over to Gitian and certified SHA256 hashes will be available for verification soon.
...
NOTE: Loki are planning to setup a gitian repository soon.
Make a pull request (both the `.assert` and `.assert.sig` files) to the TBD repository:
...
**Note:** Please ensure your gpg public key is available to check signatures by adding it to the TBD signature directory in a pull request.
