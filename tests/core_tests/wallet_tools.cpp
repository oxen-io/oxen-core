...
void wallet_accessor_test::process_parsed_blocks(
...
void wallet_tools::process_transactions(
        tools::wallet2* wallet,
        const std::vector<test_event_entry>& events,
        const cryptonote::block& blk_head,
        block_tracker& bt,
        const std::optional<crypto::hash>& blk_tail) {
    map_hash2tx_t mtx;
...
void wallet_tools::process_transactions(
    ...
    for (auto bl : blockchain) {
        ...
        wallet_tools::gen_block_data(
                bt,
                bl,
                mtx,
                v_bche.back(),
                v_parsed_block.back(),
                idx == 1 ? start_height : height);
    }

    if (wallet)
        wallet_accessor_test::process_parsed_blocks(
                wallet, start_height, v_bche, v_parsed_block, blocks_added);
}
...
bool wallet_tools::fill_tx_sources(
        ...
        size_t mixin,
        const std::optional<size_t>& num_utxo,
        const std::optional<uint64_t>& min_amount,
        block_tracker& bt,
        std::vector<size_t>& selected,
        uint64_t cur_height,
        ssize_t offset,
        int step,
        const std::optional<fnc_accept_tx_source_t>& fnc_accept) {
...
#define EVAL_BRK_COND()                        \
    ...
    for (ssize_t i = roffset; iters < ntrans && !abort; i += step, ++iters) {
        ...
        if (selected_kis.find(td.m_key_image) != selected_kis.end()) {
            oxen::log::error(
                    globallogcat,
                    "Should not happen (selected KI): {} ki: {}",
                    i,
        ...
        try {
            ...
            // Acceptor function
            if (fnc_accept) {
                tx_source_info_crate_t c_info{
                        .td = &td,
                        .src = &src,
                        .selected_idx = &selected_idx,
                        .selected_kis = &selected_kis,
                        .ntrans = ntrans,
                        .iters = iters,
                        .sum = sum,
                        .cur_utxo = cur_utxo};
            ...
            oxen::log::debug(
                    globallogcat,
                    "Selected {} from tx: {} ki: {} amnt: {} rct: {} glob: {}",
                    i,
                    dump_keys(td.m_txid.data()),
                    dump_keys(td.m_key_image.data()),
                    td.amount(),
                    td.is_rct(),
            ...
            oxen::log::trace(
                    globallogcat,
                    "Output {}, from: {} amnt: {}, rct: {}, glob: {} is not applicable: {}",
                    i,
                    dump_keys(td.m_txid.data()),
                    td.amount(),
                    td.is_rct(),
...
void wallet_tools::gen_tx_src(
    ...
    bt.get_fake_outs(
            mixin, td.is_rct() ? 0 : td.amount(), td.m_global_output_index, cur_height, outs);
    ...
    std::sort(
            src.outputs.begin(),
            src.outputs.end(),
            [&](const cryptonote::tx_source_entry::output_entry i0,
                const cryptonote::tx_source_entry::output_entry i1) {
...
void wallet_tools::gen_block_data(
        block_tracker& bt,
    ...
    for (const auto& h : bl->tx_hashes) {
        ...
        CHECK_AND_ASSERT_THROW_MES(
                mtx.end() != cit, "block contains an unknown tx hash @ {}, {}", height, h);
...
void wallet_tools::compute_subaddresses(
    ...
    const std::vector<crypto::public_key> pkeys =
            hwdev.get_subaddress_spend_public_keys(creds.get_keys(), account, 0, minors);
...
bool construct_tx_to_key(
    ...
    fill_tx_destinations(
            sender_wallet->get_account(), get_address(to), amount, fee, sources, destinations);
...
bool construct_tx_to_key(
    ...
    fill_tx_destinations(
            sender_wallet->get_account(), destinations, fee, sources, all_destinations, true);
    return construct_tx_rct(
            sender_wallet,
            sources,
            all_destinations,
...
bool construct_tx_rct(
    ...
    return construct_tx_and_get_tx_key(
            sender_wallet->get_account().get_keys(),
            subaddresses,
            sources,
            destinations_copy,
            change_addr,
