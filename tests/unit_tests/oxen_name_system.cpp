...
TEST(oxen_name_system, name_tests) {
    ...
    name_test const lokinet_names[] = {
            ...
            {"xn--fwg93vdaef749it128eiajklmnopqrstu7dwaxyz0a1a2a3a643qhok169a.loki",
             true},  // ⸘🌻‽💩🤣♠♡♢♣🂡🂢🂣🂤🂥🂦🂧🂨🂩🂪🂫🂬🂭🂮🂱🂲🂳🂴🂵🂶🂷🂸🂹
            {"abcdefghijklmnopqrstuvwxyz123456.loki",
             true},  // Max length = 32 if no hyphen (so that it can't look like a raw address)
            {"a-cdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz0123456789a.loki",
             true},  // Max length = 63 if there is at least one hyphen
            ...
            {"a-cdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz0123456789ab.loki",
             false},  // Too long with hyphen
            {"xn--fwg93vdaef749it128eiajklmnopqrstu7dwaxyz0a1a2a3a643qhok169ab.loki",
             false},                  // invalid (punycode and DNS name parts max at 63)
            {"ab--xyz.loki", false},  // Double-hyphen at chars 3&4 is reserved by DNS (currently
                                      // only xn-- is used).
    };
    ...
    for (uint16_t type16 = 0; type16 < static_cast<uint16_t>(ons::mapping_type::_count); type16++) {
        ...
        size_t names_count = ons::is_lokinet_type(type) ? oxen::char_count(lokinet_names)
                                                        : oxen::char_count(session_wallet_names);

        for (size_t i = 0; i < names_count; i++) {
            name_test const& entry = names[i];
            ASSERT_EQ(ons::validate_ons_name(type, entry.name), entry.allowed)
                    << "Values were {{type={}, name=\"{}\"}}"_format(type, entry.name);
        }
    }
}
