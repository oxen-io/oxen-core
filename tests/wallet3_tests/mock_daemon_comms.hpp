...
class MockDaemonComms : public DefaultDaemonComms {
  public:
    ...
    std::shared_ptr<oxenmq::OxenMQ> get_omq() { return std::make_shared<oxenmq::OxenMQ>(); }

    std::future<std::vector<Decoy>> fetch_decoys(
            const std::vector<int64_t>& indexes, bool with_txid = false) override {
        ...
        for (auto index : indexes) {
            auto it = std::find_if(
                    predetermined_decoys.begin(),
                    predetermined_decoys.end(),
                    [index](const auto& decoy) { return decoy.global_index == index; });
    ...
    void add_decoy(uint64_t global_index, std::string_view public_key, std::string_view mask) {
