...
TEST_CASE("DB Schema", "[wallet,db]") {
    ...
    SECTION("Insert and fetch block") {
        ...
        std::string hash;
        REQUIRE_NOTHROW(
                hash = db.prepared_get<std::string>("SELECT hash FROM blocks WHERE height = 42"));
    ...
    SECTION("Insert and fetch transaction") {
        ...
        REQUIRE_NOTHROW(
                db.prepared_exec("INSERT INTO transactions VALUES(?,?,?);", 42, 0, "footx"));

        std::tuple<std::string, int> res;
        REQUIRE_NOTHROW(
                res = db.prepared_get<std::string, int>("SELECT hash,block FROM transactions WHERE "
                                                        "id = 42"));
    ...
    SECTION("Insert and fetch key image") {
        ...
        std::string image;
        REQUIRE_NOTHROW(
                image = db.prepared_get<std::string>("SELECT key_image FROM key_images WHERE id = "
                                                     "0"));
...
TEST_CASE("DB Triggers", "[wallet,db]") {
    ...
    REQUIRE_NOTHROW(db.prepared_exec("INSERT INTO key_images VALUES(?,?);", 0, "key_image"));
    REQUIRE_NOTHROW(db.prepared_exec(
    ...
    SECTION("Confirm spend insert triggers") {
        ...
        REQUIRE(db.prepared_get<int64_t>("SELECT spent_height FROM outputs WHERE key_image = 0") ==
                1);
    }
