...
class MockKeyring : public Keyring {
  public:
    ...
    void add_key_index_pair_as_ours(
    ...
    virtual crypto::key_derivation generate_key_derivation(
            const crypto::public_key& tx_pubkey) const override {
    ...
    virtual std::vector<crypto::key_derivation> generate_key_derivations(
            const std::vector<crypto::public_key>& tx_pubkeys) const override {
    ...
    virtual std::pair<uint64_t, rct::key> output_amount_and_mask(
        ...
        for (const auto& [our_key, our_index, our_amount, sub_index, our_mask] : ours) {
            if (our_key == reinterpret_cast<const crypto::public_key&>(derivation) &&
                our_index == i)
                return {our_amount, our_mask};
        }
        throw std::invalid_argument{
                "mock_keyring, output_amount_and_mask called on output that isn't ours"};
    }
