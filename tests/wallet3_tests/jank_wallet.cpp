...
int main(int argc, char** argv) {
    ...
    std::string wallet_addr =
            "T6SYSC9FVpn15BGNpYYx3dHiATyjXoyqbSGBqgu5QbqEUmETnGSFqjtay42DBs6yZpVbgJcyhsbDUcUL3msN4G"
            "yW2HhR7aTmh";
    if (argc == 1 or std::string(argv[1]) == "1")  // no cli arg = test wallet 1
    {
        tools::load_from_hex_guts(
                "d6a2eac72d1432fb816793aa7e8e86947116ac1423cbad5804ca49893e03b00c"sv, spend_priv);
        tools::load_from_hex_guts(
                "2fc259850413006e39450de23e3c63e69ccbdd3a14329707db55e3501bcda5fb"sv, spend_pub);

        tools::load_from_hex_guts(
                "e93c833da9342958aff37c030cadcd04df8976c06aa2e0b83563205781cb8a02"sv, view_priv);
        tools::load_from_hex_guts(
                "5c1e8d44b4d7cb1269e69180dbf7aaf9c1fed4089b2bd4117dd1a70e90f19600"sv, view_pub);
    } else  // cli arg that isn't "1" = test wallet 2
    {
        wallet_name = "test_wallet2";
        wallet_addr =
                "T6ThzoXPCEvKjB9jce9rhd5gpovK5hvfX6yE1d6nC2H7QobLgoYNvW12qrahC9HtrtHNty58UXHZoNGyDn"
                "CeSFbY3Bu8yFtgh";
        tools::load_from_hex_guts(
                "e6c9165356c619a64a0d26fafd99891acccccf8717a8067859d972ecd8bcfc0a"sv, spend_priv);
        tools::load_from_hex_guts(
                "b76f2d7c8a036ff65c564dcb27081c04fe3f2157942e23b0496ca797ba728e4f"sv, spend_pub);
        tools::load_from_hex_guts(
                "961d67bb5b3ed1af8678bbfcf621f9c15c2b7bff080892890020bdfd47fe4f0a"sv, view_priv);
        tools::load_from_hex_guts(
                "8a0ebacd613e0b03b8f27bc64bd961ea2ebf4c671c6e7f3268651acf0823fed5"sv, view_pub);
    }

    std::cout << "Loading wallet \"" << wallet_name << "\" with address " << wallet_addr << "\n";

    auto keyring = std::make_shared<wallet::Keyring>(
    ...
    conn = oxenmq->connect_remote(
            remote, [](auto) {}, [](auto, auto) {});
    ...
    std::thread exit_thread([&]() {
        while (not done) {
            ...
            if (args[0] == "send") {
                ...
                std::cout << "after block " << scan_height << ", " << wallet_name
                          << " balance is: " << wallet->get_balance() << "\n";
    ...
    while (not done) {
        ...
        if ((not printed_synced) and current_chain_height > 0 and
            last_scan_height == current_chain_height) {
            std::cout << "syncing appears finished, " << wallet_name
                      << "height = " << last_scan_height << ", balance = " << wallet->get_balance()
                      << "\n";
            printed_synced = true;
        }
